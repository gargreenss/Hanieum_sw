"""
AI 스마트 식사 보조 시스템 - 로컬 WebSocket 서버 (v19 통합판)

[합친 것]
- 기존 server.py : WebSocket 수신, 파이 응답(JSON), 디버그 저장/창, 쿨다운
- webcam_test v19 : 끝 클래스(top/s_top/f_top) 직접 탐지, 흔들림 보정(EMA),
                    흐림(블러) 검사, MOVE_RESET(같은 자리 재호출 방지)

[수정 이력]
- food_recognizer import 경로 수정 (haiku_test 폴더 참조)
- 판별 불확실("알 수 없는 음식"/"없음") 시 안내 보류 분기 추가

구조:
Raspberry Pi 5 → JPEG/WebSocket → 이 서버
  → YOLO(v19): 식기 '끝' 클래스 탐지 → 끝점 흔들림 보정
  → 멈춤 + 쿨다운 + 위치이동 조건 → 크롭 → 흐림 검사
  → Claude Haiku 판별 → JSON 반환 → 파이 TTS

필요: pip install ultralytics opencv-python websockets numpy anthropic pillow python-dotenv
실행: python server_m.py
※ EC2 배포 시 SHOW_YOLO_WINDOW = False 필수!
"""
import asyncio
import base64
import json
...
# food_recognizer는 haiku_test 폴더에 있음 → import 경로 추가
sys.path.insert(0, str(BASE.parent / "haiku_test"))
...
# ============================================================
# models/best.pt 우선, 없으면 같은 폴더 best.pt
...
# ============================================================
YOLO_CONF = 0.40
STOP_FRAMES = 6          # 최근 N프레임으로 멈춤 판단
STOP_PIXELS = 65         # 이동량이 이 픽셀 미만이면 정지
COOLDOWN = 4.0           # Haiku 재호출 금지 시간(초)
CROP_RATIO = 0.35        # 크롭 크기 = 짧은 변 * 비율
MISS_TOLERANCE = 5       # 탐지 깜빡임 허용 프레임
SMOOTH_ALPHA = 0.3       # 끝점 흔들림 보정(EMA)
MOVE_RESET = 80          # 직전 판별 위치에서 이만큼 벗어나야 재호출
BLUR_THRESHOLD = 100     # 선명도 기준 (Laplacian, 로그 보고 조정)
TIP_OFFSET = (-0.25, 0.25)   # 빨간 점 보정: 박스 크기 대비 (x비율, y비율)
                             # 박스가 커서 점이 위로 치우침 → 왼쪽 아래로 밀기
                             # 덜 가면 -0.35/0.35, 너무 가면 -0.15/0.15로 조정

# 판별 불확실 응답 → 안내 보류 대상
UNCERTAIN_ANSWERS = ("알 수 없는 음식", "없음")
...
SHOW_YOLO_WINDOW = True   # ★ EC2에서는 반드시 False
...
class ClientState:
    def __init__(self):
        ...
        self.last_pos = None      # 마지막 판별 위치 (MOVE_RESET용)
        self.prev_tip = None      # 흔들림 보정용 직전 끝점
        self.miss_count = 0
        self.frame_count = 0
...
def get_tip(results):
    ...
    # ── 빨간 점 보정: 박스 중심을 박스 크기 비율만큼 왼쪽 아래로 ──
    cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
    cx += int((x2 - x1) * TIP_OFFSET[0])   # 왼쪽으로 박스폭의 25%
    cy += int((y2 - y1) * TIP_OFFSET[1])   # 아래로 박스높이의 25%
    return {
        "center": (cx, cy),
        "box": (x1, y1, x2, y2),
...
def process_frame(frame, state):
    ...
    if tip_info:
        ...
        if is_stopped(state) and (now - state.last_trigger > COOLDOWN) and far_enough:
            ...
            if crop.size != 0:
                ...
                if not sharp:
                    ...
                    # last_trigger 갱신 안 함 → 다음 프레임에 재시도
                else:
                    state.last_trigger = now
                    state.last_pos = tip

                    if SAVE_DEBUG:
                        cv2.imwrite(str(DEBUG_DIR / f"crop_{state.frame_count:06d}_{int(now)}.jpg"), crop)
                    ...
                    try:
                        ...
                        # ── 판별 불확실 시 안내 보류 ──
                        if r.get("food") in UNCERTAIN_ANSWERS:
        ...
        if state.miss_count > MISS_TOLERANCE:
            state.recent.clear()
            state.prev_tip = None   # v19: 놓치면 보정도 리셋
...
async def main():
    async with websockets.serve(handler, HOST, PORT, max_size=None):
        print()
        print("=" * 65)
        print("AI 스마트 식사 보조 로컬 서버 (v19 통합판)")
        print("=" * 65)
